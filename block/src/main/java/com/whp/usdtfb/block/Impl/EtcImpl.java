...
import com.whp.usdtfb.block.Utils.Web3jClientEtc;
 ...
 */
@Service
public class EtcImpl implements EtcInterface {
    ...
    private static Web3j web3j = Web3jClientEtc.getClient();
    private static Admin admin = Web3jClientEtc.getAdmin();
    private int gaodu = 0;
    ...
    @Override
    public void XunHuan() {
        String value = RedisUtils.get("ETC", 7);
        if (value == null || value == "") {
            RedisUtils.set("ETC", gaodu + "", 7);
        } else {
            gaodu = Integer.parseInt(value);
        }
        BigInteger bigInteger = getCurrentBlockNumber();
        System.out.println("高度：" + bigInteger);
        List<Map<String, Object>> wallet_list = fbWalletDao.FbWalletList(2);
            ...
            if (ethBlock != null) {
                ...
                list.forEach(tx -> {
                    ...
                    for (Map<String, Object> map : wallet_list) {
                        if (transaction.getTo().equals(map.get("address"))) {
                            //获得交易
    ...
    public void zhuanzhang(String address, String hash, String from_address, BigDecimal value, int block, String private_name, String userid) {
        try {
            ...
            if (value.compareTo(money) != -1) {
                ...
                json.put("codeid", 2001);
                fbUsdtDao.FbUsdtInsert(json);
                Map<String, Object> sys_wallet = fbSysWalletDao.FbSysWalletSelect(2);
